...
local function match_client(c, forced_tags, hints)
    ...
    local tmp,tag = c_rules.class[low_c],awful.tag.add(get_class(c),{name="D",onetimer=true,volatile=true,exclusive=true,screen=(c.screen.index <= capi.screen.count())
    ...
    tmp.tags[#tmp.tags+1] = {name="D",instances = setmetatable({[get_screen_idx(c.screen)]=tag}, { __mode = 'v' }),volatile=true,screen=c.screen,exclusive=true}
    c:tags({tag})
    return module.focus_client(c,props)
end
