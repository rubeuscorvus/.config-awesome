...
local volume = lain.widget.pulsebar({
      ticks = false, width = 70, 
      colors = { background = "#33393b", mute = "#91231c", unmute = "#7861b0"
...
local volumebg = wibox.container.background(volume.bar, "#7861b0", gears.shape.rectangle)
local volumewidget = wibox.container.margin(volumebg, 5, 10, 4, 4)

local volumeicon = awful.widget.watch('pacmd list-sinks', 0.5, function(widget, stdout)
					 widget:set_font("github-octicons 16")
					 for line in stdout:gmatch("[^\r\n]+") do
					    if line:match("muted: no") then
					       widget:set_text("")
					       return
					    elseif  line:match("muted: yes") then
					       widget:set_text("")
					        return
					    end
					     collectgarbage()
...
mytextclock = awful.widget.textclock(" %H:%M, %d %b")
...
awful.screen.connect_for_each_screen(function(s)
					 ...
					 tasklist_buttons,
    {
        ...
        shape_border_color = '#33393b',
	shape = gears.shape.octogon,
    })
    
    -- @DOC_WIBAR@
    -- Create the wibox
    s.mywibox = awful.wibar({ position = "top", screen = s, height=25, border_width=5, border_color = "#33393b" })
...
local autostart = require("rc.autostart")
